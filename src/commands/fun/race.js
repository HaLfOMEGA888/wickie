...
module.exports = new Command({
  ...
  async run(message, args, client) {
    ...
    message.channel.send(
      "🏁 Race has begun! Type `join race` to join the race! 🏁\nThe race will begin in 60 seconds!"
    );
    ...
    collector.on("collect", (m) => {
      if (!participants.includes(m.author.id)) {
        if (participants.length >= 25)
          return message.channel.send("Maximum 25 people can join a race");
    ...
    collector.on("end", async () => {
      if (participants.length < 2) {
        return message.channel.send("Not enough ppl joined, get friends lol");
      }
      ...
      message.channel.send(
        ` ${participants.length} ppl joined. Participants - ${players}`
      );
